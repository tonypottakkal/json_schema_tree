...
// Transform JSON schema to hierarchical format
function transformSchemaToHierarchy(schema) {
    let root = {
        name: schema["$id"] || schema.title || "Root",
        children: []
    };

    function processProperties(properties, parentNode) {
        for (let key in properties) {
            ...
            if (properties[key].type === "object" && properties[key].properties) {
                processProperties(properties[key].properties, childNode);
            } else if (properties[key].type === "array" && properties[key].items && properties[key].items.properties) {
                processProperties(properties[key].items.properties, childNode);
            }
    ...
    processProperties(schema.properties, root);
    return root;
}

// function to clear the visualization
function clearVisualization() {
    d3.select('#chart svg').remove(); // Remove the existing SVG to reset the visualization
}



//this is the main function that will be called from the index.html
document.getElementById('jsonFileInput').addEventListener('change', function() {
    if (this.files.length > 0) {
        const file = this.files[0];
        const reader = new FileReader();
        reader.onload = (e) => {
            clearVisualization(); // Clear the visualization here
            const jsonSchema = JSON.parse(e.target.result);
            const hierarchicalData = transformSchemaToHierarchy(jsonSchema);
            console.log("Transformed Hierarchical Data:", hierarchicalData);
            parentFunction(hierarchicalData); // Proceed to visualize the new data

            // Reset the file input to allow re-uploading the same file or a new one
...
// function to create the tree diagram from the transformed data
